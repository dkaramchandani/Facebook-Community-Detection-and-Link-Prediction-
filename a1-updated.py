...
def bfs(graph, root, max_depth):
    ...
    """
    ###TODO
    # initialize a list with flag that all the nodes are not visited
    ...
    # add the root node & update the relevant details
    q.append(root)
    visited[root] = True
    node2distance[root] = 0
    node2num_paths[root] = 1

    while len(q) > 0: #n times
        currNode = q.popleft()
        dis = node2distance[currNode]
        paths = node2num_paths[currNode]
        for currNodeChild in graph.neighbors(currNode): #~2E times
            if dis < max_depth:
                if visited[currNodeChild] == False:
                    visited[currNodeChild] = True
                    node2distance[currNodeChild] = node2distance[currNode] + 1

                if node2distance[currNodeChild] == dis + 1 and paths != 0:
                    node2num_paths[currNodeChild] = node2num_paths[currNodeChild] + paths
                    node2parents[currNodeChild].append(currNode)
                    q.append(currNodeChild)

    return node2distance,node2num_paths, node2parents
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    ###TODO
    #pass
    weightNodes = defaultdict(lambda: 1.0)
    weightEdges = defaultdict(float)

    sortedNodeDistances = sorted(node2distances.items(), key=lambda k: (-k[1]))
    for currNode in sortedNodeDistances:
        weightCurrNode = weightNodes[currNode[0]]
        if currNode[0] != root:
            parentCurrNode = node2parents[currNode[0]]
            if len(parentCurrNode) == 0:
                break

            if len(parentCurrNode) == 1:
                weightNodes[parentCurrNode[0]] += weightCurrNode
                weightEdges[tuple(sorted([currNode[0], parentCurrNode[0]]))] = weightCurrNode
            else:
                weightCurrEdge = weightCurrNode / len(parentCurrNode)
                for p in parentCurrNode:
                    weightNodes[p] += weightCurrEdge
                    weightEdges[tuple(sorted([currNode[0], p]))] = weightCurrEdge
        else:
            break
...
def approximate_betweenness(graph, max_depth):
    ...
    ###TODO
    #pass
    defdictEdgeVals = defaultdict(int)
    for currNode in graph.nodes():
        nd, nnp, np = bfs(graph, currNode, max_depth)
        dictEdgeCreditVal = bottom_up(currNode, nd, nnp, np)
        for edgeCreditVal in dictEdgeCreditVal:
            defdictEdgeVals[edgeCreditVal] = defdictEdgeVals.get(edgeCreditVal, 0) + dictEdgeCreditVal[edgeCreditVal]

    for k in defdictEdgeVals:
        defdictEdgeVals[k] /= 2

    return defdictEdgeVals
...
def is_approximation_always_right():
    ...
    Ans: No, if we consider an example where the edge with highest betweenness was at level 3 of BFS traversal,
    then approximate_betweeness may give a different value than the exact if there is not much difference in values
    for the next 2 edges with high betweenness.
...
def partition_girvan_newman(graph, max_depth):
    ...
    ###TODO
    d = graph.copy()
    edgePartitions = sorted(approximate_betweenness(d, max_depth).items(), key=lambda x:(-x[1], x[0]))

    subComponents = [c for c in nx.connected_component_subgraphs(d)]
    lstRemoveEdges = []

    if len(subComponents)==1:
        for p in edgePartitions:
            d.remove_edge(*p[0])
            subComponents = [c for c in nx.connected_component_subgraphs(d)]
            if len(subComponents) > 1:
                break
    subComponents[0],subComponents[1]= subComponents[1],subComponents[0]
    subComponents = sorted(subComponents, key=lambda x: sorted(x.nodes())[0])
...
def get_subgraph(graph, min_degree):
    ...
    for s in graph.nodes():
        if (graph.degree(s) < min_degree):
            sub_graph.remove_node(s)
    return sub_graph
...
def volume(nodes, graph):
    ...
    ###TODO
    volumeofnodes = 0
    l = sorted(list(graph.nodes()))
    for n in sorted(nodes):
        for nn in l:
            volumeofnodes += graph.number_of_edges(n, nn)
        l.remove(n)

    return volumeofnodes
...
def score_max_depths(graph, max_depths):
    ...
    ls = []
    for c in max_depths:
        components = partition_girvan_newman(graph, c)
        norm_value = norm_cut(set(components[0].nodes()), set(components[1].nodes()), graph)
        ls.append(tuple([c, norm_value]))
    return ls
## Link prediction
...
def make_training_graph(graph, test_node, n):
    ...
    ls = sorted(graph.neighbors(test_node))
    i = 0
    while n > 0 and i in range(0, len(ls)):
        training_graph.remove_edge(test_node, ls[i])
        n -= 1
        i += 1
...
def path_score(graph, root, k, beta):
    ...
    ###TODO
    pathScores = []
    n = graph.neighbors(root)
    nodeScores = [x for x in graph.nodes() if x not in n and x != root]
    spl, spn, node2parents = bfs(graph, root, 5)
    for n in nodeScores:
        spl2 = spl[n]
        spn2 = spn[n]
        sc = ((root, n), beta ** spl2 * spn2)
        pathScores.append(sc)

    return sorted(pathScores, key=lambda x: (-x[1], x[0][1]))[:k]
...
def evaluate(predicted_edges, graph):
    ...
    """
    ###TODO
    prediction, total_edges = 0, 0
    for a, b in predicted_edges:
        if graph.has_edge(a, b):
            prediction += 1
        total_edges += 1
    return (prediction/len(predicted_edges))
